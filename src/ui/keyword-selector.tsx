...
export default function KeywordSelector({ selected, onChange }: KeywordSelectorProps) {
  ...
  return (
    <section aria-label="리뷰 키워드 선택" className="w-[80%] mx-auto space-y-4 whitespace-nowrap ">
      {selected.length > 0 && (
        <div>
          <h3 className="font-medium text-gray-700 mb-2">선택된 키워드</h3>
          <ul className="flex flex-wrap gap-2">
            {selected.map((keyword) => (
              <li
                key={keyword}
                className="w-fit text-xs px-3 py-2 bg-white text-gray-700 border border-neutral-200 hover:bg-gray-100 rounded-full"
              >
                {keyword}
                <button
          ...
          <ul className="flex flex-wrap justify-items-start gap-2 pt-4">
            {keywords.map((keyword) => {
              ...
              const buttonClass = isSelected
                ...
                : "w-fit text-xs px-3 py-2 bg-white text-gray-700 border border-neutral-200 hover:bg-gray-100 rounded-full";
